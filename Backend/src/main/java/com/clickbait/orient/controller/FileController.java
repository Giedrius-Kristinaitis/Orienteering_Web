...
public class FileController {
     ...
     */
    @PostMapping("/photo/{eventId}/{teamId}/{checkpointId}")
    public ResponseEntity<Photo> uploadPhoto(@RequestParam("file") MultipartFile file, @PathVariable String eventId, @PathVariable String teamId, @PathVariable String checkpointId) {
        String downloadURL = service.savePhoto(file, eventId, teamId, checkpointId);

        if (downloadURL == null) {
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        }

        Photo response = new Photo(eventId, teamId, checkpointId, downloadURL, file.getContentType(), file.getSize());
